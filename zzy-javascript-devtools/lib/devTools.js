  ...
  _createClass(DevTools, [{
    ...
    */
    function generateTree(items) {
      ...
      var selectVal = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'parentId';
      return items.filter(function (item) {
        return item[selectVal] === parentId;
      }).map(function (i) {
        return _objectSpread(_objectSpread({}, i), {}, {
          children: _this.generateTree(items, i.id, selectVal)
        });
      });
    ...
    key: "allEquals",
    value: function (_allEquals) {
      function allEquals(_x, _x2) {
        return _allEquals.apply(this, arguments);
      }

      allEquals.toString = function () {
        return _allEquals.toString();
      };
    ...
    key: "getDaysDiffBetweenDates",
    value: function getDaysDiffBetweenDates(dateInitial, dateFinal) {
      return (dateFinal - dateInitial) / (1000 * 3600 * 24);
    } // 时间格式转化年 -- 接受 ms 级的时间戳

  }, {
    key: "changeTimeYear",
    value: function changeTimeYear(time) {
    ...
    key: "formateSeconds",
    value: function formateSeconds(surPlusTime) {
      var secondTime = parseInt(surPlusTime); //将传入的秒的值转化为Number
      ...
      result = "".concat(d.toString().padStart(2, '0'), ":").concat(h.toString().padStart(2, '0'), ":").concat(min.toString().padStart(2, '0'), ":").concat(secondTime.toString().padStart(2, '0'));
      return result; // return 00:00:00:00
    } // 时间戳转化为当前时间 

  }, {
    key: "formatNowTime",
    value: function formatNowTime(time) {
      function add0(m) {
        return m < 10 ? '0' + m : m;
      }
    ...
    key: "bottomVisible",
    value: function bottomVisible() {
      return document.documentElement.clientHeight + window.scrollY >= (document.documentElement.scrollHeight || document.documentElement.clientHeight);
    } // 检查当前标签页是否活动

  }, {
    key: "isBrowserTabFocused",
    value: function isBrowserTabFocused() {
    ...
    key: "smoothScroll",
    value: // 滚动到指定元素区域 --> 该代码段可将指定元素平滑滚动到浏览器窗口的可见区域。
    ...
    key: "preventScroll",
    value: function preventScroll(scrollNum) {
      // 存储当前滚动位置
      scrollNum = window.scrollY; // 将可滚动区域固定定位，可滚动区域高度为 0 后就不能滚动了

      document.body.style["overflow-y"] = "hidden";
      document.body.style.position = "fixed";
      document.body.style.width = "100%";
      document.body.style.top = -scrollNum + "px"; // document.body.style['overscroll-behavior'] = 'none'
    } // 恢复滚动条  接受一个y轴 Number  如果配合 preventScroll 方法使用需要现将 固定前的滚动条高度记录，再恢复时赋值给 recoverScroll 方法

  }, {
    key: "recoverScroll",
    value: function recoverScroll(scrollNum) {
      document.body.style["overflow-y"] = "auto";
      document.body.style.position = "static"; // document.querySelector('body').style['overscroll-behavior'] = 'none'
    ...
    key: "elementIsVisibleInViewport",
    value: function elementIsVisibleInViewport(el) {
    ...
    value: function launchFullscreen(el) {
    ...
    */
    ...
    key: "formatFileSize",
    value: function formatFileSize(fileSize) {
      var temp;
    ...
    key: "isAndroidPlatform",
    value: function isAndroidPlatform() {
    ...
    key: "uploadImage",
    value: function uploadImage(_ref3) {
      var url = _ref3.url,
          file = _ref3.file;
    ...
    value: function setDomRem() {
      ...
      (function (designWidth, maxWidth) {
        ...
        function refreshRem() {
          ...
          var rem = width * +num / designWidth; //兼容UC开始
    ...
    key: "rewirteLog",
    value: function rewirteLog() {
      console.log = function () {};
    } // 增强版typeOf typeOf 关键字对于null，date 都会认为是Object，不准确

  }, {
    key: "typeOf",
    value: function typeOf(data) {
      var res = Object.prototype.toString.call(data).split(' ')[1];
      return res.slice(0, res.length - 1);
    } // 深拷贝

  }, {
    key: "deepClone",
    value: function (_deepClone) {
    ...
    value: function lazyImage() {
      ...
      var imageElements = Array.prototype.slice.call(className ? document.querySelectorAll(className) : document.getElementsByTagName('img')),
          _throttleFn; // 只针对具有参数的值进行处理


      imageElements = imageElements.filter(function (item) {
    ...
    value: function throttle(func, wait, options) {
      var timeout, context, args; // 上一次的时间点
      ...
      var throttled = function throttled() {
        // 拿到当前时间的时间戳
        var now = new Date().getTime(); // 如果 上一次时间为0 且 不设置立即执行，将当前时间赋值给上次时间

        if (!previous && options.leading === false) previous = now; // 当前空档时间 设定等待值 - (当前 - 上一次时间)

        var remaining = wait - (now - previous);
        context = this;
        args = arguments; // console.log(arguments, 'arguments');
    ...
    key: "infinityScrolling",
    value: function infinityScrolling(dom, cb) {
      var _this2 = this;

      var that = this;
      this.infinityScrolling.bol = true;
      var throttleFn; //  需要一个外界bol判断是否加载完毕 不完毕不再次触发

      if ('IntersectionObserver' in window) {
        var intersectionOb = new IntersectionObserver(function (entry) {
          if (!_this2.infinityScrolling.bol) return; // 1 - 可视范围内 0 - 不在可视范围内
        ...
        // 启用节流 100ms
        throttleFn = this.throttle(monitorScroll, 100); // 开启监听
      ...
      this.infinityScrolling.closeMonitor = function () {
        if ('IntersectionObserver' in window) return; // 关闭节流

        throttleFn.cancel(); // 取消监听
      ...
      function monitorScroll() {
        if (!that.infinityScrolling.bol) return; // 检查指定的元素在视口中是否可见
    ...
    key: "deleteHTMLTags",
    value: function deleteHTMLTags(str) {
    ...
    key: "getDomStyle",
    value: function getDomStyle(el, ruleName) {
    ...
    */
    ...
    key: "JSB_appMethod",
    value: function JSB_appMethod(name) {
    ...
    key: "JSB_jsMethod",
    value: function JSB_jsMethod(name) {
      return new Promise(function (reslove, reject) {
        if (!window.setupWebViewJavascriptBridge) return reject('请先将 JSBriged.js 引入！');
        window.setupWebViewJavascriptBridge(function (bridge) {
