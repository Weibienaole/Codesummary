...
const buildZislands = async (
  ...
  ${Object.entries(zislandPathToMap)
    ...
    .join('')}
window.ISLANDS = { ${Object.keys(zislandPathToMap).join(', ')} };
...
async function renderPage(
  ...
  await Promise.all(
    routes.map(async (route) => {
      ...
      const { appHtml, zislandToPathMap, propsData } = await render(routePath)

      await buildZislands(root, zislandToPathMap)
      const html = `
    <!DOCTYPE html>
    <html>
      <head> 
